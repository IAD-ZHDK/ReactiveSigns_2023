...
function draw() {
  ...
  image(images[i],poster.screens[0].x,0,width,height);
...
function getindex(vector) {
  let normal = map(vector.x, 0.0, 1.0,0.0+sin(frameCount * 0.1)*0.02,1.0+sin(frameCount * 0.1)*0.02) // make sure we really get to the first and last frame 
  let max = imagCount-1
